...
fn init_stamps_machine_owned_provenance() {
    ...
    // Generated artifacts: AGENTS.md first-line HTML comment + graph.trig comment.
    ...
    let trig = std::fs::read_to_string(s.path().join("situation/graph.trig")).unwrap();
    assert!(
        trig.starts_with(&format!(
            "# generated by bedrock v{v}; do not edit; source: situation/"
        )),
        "graph.trig opens with the machine-owned comment: {trig}"
    );
}
